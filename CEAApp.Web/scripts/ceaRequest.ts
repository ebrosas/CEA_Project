...
class CEADetail {
    ...
    originatorEmpNo: number;
    originatorEmpName: string;
    createDate: string;
    createdByEmpNo: number;
    ...
    equipmentParentNo: string;
    equipmentParentDesc: string;
    dateofComission: string;
...
interface IFileAttachment {
    requisitionAttachmentID: number;
    requisitionID: number;
    ...
    sequenceNo: number;
    base64File: string;
    base64FileExt: string;
}

interface WorkflowParameter {
    readonly requisitionNo: string;
    readonly wfInstanceID: string;
    appRole?: number;
    appRoutineSeq?: number;
    appApproved?: boolean;
    appRemarks?: string;
    approvedBy?: number;
    approvedName?: string;
    statusCode?: string;
    currentAssignedEmpNo?: number;
    reassignedEmpNo?: number;
    reassignedEmpName?: string;
    reassignedEmpEmail?: string;
    routineSeq?: number;
    onHold?: boolean;
    reason?: string;
    reassignedBy?: number;
    reassignedName?: string;
    ceaDescription?: string;
}
//#endregion
...
enum ModalFormTypes {
    ...
    Reject = "reject",
    Cancel = "cancel",
    InvalidEstimatedCost = "invalid_cost",
    RecallRequest = "recall",
    ReactivateRequest = "reactivate",
    ReopenRequest = "reopen",
    ReassignRequest = "reassign"
}
...
enum ChangeStatusActionType {
    ReactivateRequest = "ActivateRequisition",
    RecallRequest = "RecallRequisition",
    ReopenRequest = "OpenRequisition"
}
...
const expenseArray: IScheduleExpense[] = [];
const attachmentArray: IFileAttachment[] = [];
...
$(() => {
    ...
    let callerFormVal = $("#hidCallerForm").val() as string;
    ...
    if (!CheckIfNoValue($("#hidQueryString").val())) {
        baseUrl = baseUrl!.concat($("#hidQueryString").val() as string);
    }
    ...
    if (!CheckIfNoValue(attachmentSessionData)) {
        ...
        if (Array.isArray(attachmentList)) {
            attachmentList.forEach(function (value, index) {
                let attachmentItem: IFileAttachment = {
                    ...
                    fileName: value.attachmentFileName.replace(/\s+/g, ''),      // Remove spaces in the file name
    ...
    if (!CheckIfNoValue(expenseSessionData)) {
        ...
        if (Array.isArray(expenseList)) {
            expenseList.forEach(function (value, index) {
                let expenseItem: IScheduleExpense = {
                    fiscalYear: value.fiscalYear,
                    quarter: value.quarter,
                    amount: value.amount,
                    projectNo: $("input[name='ProjectNo']").val() as string,
...
function checkIfCEAAdmin() {
    ...
    if (!CheckIfNoValue($("#hidCEAAdminJSON").val())) {
        var adminList = JSON.parse($("#hidCEAAdminJSON").val() as string);
...
function loadAttachment() {
    if (!CheckIfNoValue($("#hidAttachmentJSON").val())) {
        var attachmentList = JSON.parse($("#hidAttachmentJSON").val() as string);
        if (Array.isArray(attachmentList)) {
...
function loadExpenses() {
    if (!CheckIfNoValue($("#hidExpenseJSON").val())) {
        var expenseList = JSON.parse($("#hidExpenseJSON").val() as string);
        if (Array.isArray(expenseList)) {
...
function showErrorCEAEntry(obj: any, errText: any, focusObj: any) {
    ...
    switch ($(btn)[0].id) {
        ...
        case "btnDelete":
            if (HasAccess(gUserAccess.userFrmCRUDP, FormAccessIndex.Delete)) {
                ...
                ($("#modConfirmation") as any).modal("show");                                
            }
            else
        ...
        case "btnReassign":
            ...
            ($("#modReassignment") as any).modal("show");
            break;
    }
}
...
function addExpenseItem() {
    ...
    if (!hasError) {
        ...
        let expenseItem: IScheduleExpense = {
            ...
            quarter: $("select[name='ExpenseQuarter']").val() as string,
...
function saveExpenseItem() {
    try {
        ...
        if (!hasError) {
            ...
            selecteExpenseItem.quarter = $("select[name='ExpenseQuarter']").val() as string;
...
function deleteExpenseItem() {
    try {
        ...
        let lineNo: number = GetIntValue($(this).attr("data-lineno"));
        if (lineNo > 0) {
...
function editExpenseItem() {
    ...
    let fiscalYear: number = GetIntValue($(this).attr("data-fiscalyear"));
    let quarter: string = $(this).attr("data-quarter") as string;
    let amount: number = GetFloatValue($(this).attr("data-amount"));
    let lineNo: number = GetIntValue($(this).attr("data-lineno"));      
...
function populateSchedExpenseTable(data: any) {
    var dataset = data;
...
function populateAttachmentTable(data: any) {
    var dataset = data;
...
function openFileAttachment() {
    try {
        ...
        if (!CheckIfNoValue($(this).attr("data-filename")))
            fileName = $(this).attr("fileName") as string;
    ...
    try {
        ...
        reader.onloadend = function () {
            ...
            if (!CheckIfNoValue(base64String))
                $("#hidBase64File").val(base64String as string);
        }
...
function saveAttachment() {
    try {
        ...
        if (!hasError) {
            let fileName = $("input[name='DisplayName']").val() as string;
            let fileFullName = $("#hidFileName").val() as string;
            let fileExt = fileFullName.substring(fileFullName.lastIndexOf('.'));

            let attachmentItem: IFileAttachment = {
                requisitionAttachmentID: 0, 
                requisitionID: 0,
                attachmentFileName: fileName.concat((new Date()).getTime().toString()).concat(fileExt), // Append timestamp to the filename 
                ...
                fileName: fileName.replace(/\s+/g, ''),                 // Remove spaces in the file name
                sequenceNo: attachmentArray.length + 1,                  // Store the array lenght to this field
                base64File: $("#hidBase64File").val() as string,
...
function deleteAttachment() {
    try {
        ...
        let fileName = $(this).attr("data-filename") as string;
    ...
    try {
        ...
        if (fileName.length > 0) {
            ...
            if (isFound) {
                ...
                let selectedFile: IFileAttachment = attachmentArray[itemIndex];
    ...
    switch (btnAttrib) {
        case ActionLinkTypes.ActionRecall:
            ...
            if (HasAccess(gUserAccess.userFrmCRUDP, FormAccessIndex.Update)) {
                ...
                ($("#modChangeStatus") as any).modal("show");                                
            }
            else
...
function handleModalButtonClick() {
    ...
    if (btnAttrib == ModalResponseTypes.ModalYes) {
        ...
        let requisitionNo = $("input[name='RequisitionNo']").val()?.toString();
        let empNo = GetIntValue($("input[name='UserEmpNo']").val());
        let comments = $("textarea[name='ChangeStatusComment']").val()?.toString();
        ...
        if (!hasError) {
            ...
            // Hide the modal form
            ($("#modChangeStatus") as any).modal("hide");
        ...
        let reassignedEmpName = $("input[name='ReassignEmpName']").val()?.toString();
        let reassignedEmpEmail = $("input[name='ReassignEmpEmail']").val()?.toString();        
        let currentUserEmpNo = GetIntValue($("input[name='UserEmpNo']").val());
        let currentUserEmpName = $("input[name='UserName']").val()?.toString();
        let routineSeq = 0;
        let onHold = false;
        let reason = $("textarea[name='ReassignReason']").val()?.toString();
        let ceaDescription = $("input[name='RequisitionDescription']").val()?.toString();
        ...
        if (!hasError) {
            // Hide the modal form
            ($("#modReassignment") as any).modal("hide");
        ...
        success:
            function (response, status) {
                if (status == "success") {
                    ...
                    let orignatorName = $("#hidOriginatorEmpName").val() as string;
...
function populateOriginatorList(data: any) {
    ...
    try {
        ...
        $("#txtOriginator").autocomplete({
            ...
            autoFocus: false,            // Set first item of the menu to be automatically focused when the menu is shown
        ...
        $("#txtReassignTo").autocomplete({
            source: empArray2,           // Source should be Javascript array or object
            autoFocus: false,            // Set first item of the menu to be automatically focused when the menu is shown
            minLength: 2,               // The number of characters that must be entered before trying to obtain the matching values. By default its value is 1.
            delay: 300,                 // This option is an Integer representing number of milliseconds to wait before trying to obtain the matching values. By default its value is 300.
            select: function (event, ui) {
        ...
        success:
            function (response, status) {
                if (status == "success") {
                    ...
                    let equipmentNo = $("#hidEquipmentNo").val() as string;
...
function populateEquipmentList(data: any) {
    ...
    try {
        ...
        interface IEquipment {
            label: string;
            value: string;
            parentEquipmentNo: string;
            parentEquipmentDesc: string;
        }

        const equipmentArray: IEquipment[] = [];
        var item;
        ...
        $("#txtEquipmentNo").autocomplete({
            source: equipmentArray,     // Source should be Javascript array or object
...
function populateEquipmentTable(data: any) {
    var dataset = data;
    if (dataset == "undefined" || dataset == null) {
        // Get DataTable API instance
...
function getEquipmentDetail() {
    ...
    let equipmentNo: string = $(this).attr("data-equipmentno") as string;
    let equipmentDesc: string = $(this).attr("data-equipmentdesc") as string;
    let parentEquipmentNo: string = $(this).attr("data-parentequipmentno") as string;
    let parentEquipmentDesc: string = $(this).attr("data-parentequipmentdesc") as string;
    ...
    // Hide the modal form
    ($("#modEquipment") as any).modal("hide");
}
