...
$(() => {
    ...
    let ignoreSavedData: boolean = Boolean($("input[name='IgnoreSavedData']").val());
...
function populateEmployeeList(data: any) {
...
function populateInquiryTable(data: any) {
    var dataset = data;
...
function openProjectDetail() {
    let projectNo: string = $(this).text().trim();
...
function openRequisitionDetail() {
    let reqNo: string = $(this).text().trim();
    let actionType = 0;
...
function openRequisitionStatus() {
    ...
    if (!CheckIfNoValue($(this).attr("data-requisitionid")))
        requisitionID = $(this).attr("data-requisitionid") as string;

    if (!CheckIfNoValue($(this).attr("data-projectno")))
        projectNo = $(this).attr("data-projectno") as string;

    if (!CheckIfNoValue($(this).attr("data-requisitionno")))
        requisitionNo = $(this).attr("data-requisitionno") as string;

    if (!CheckIfNoValue($(this).attr("data-createdno")))
        createdNo = $(this).attr("data-createdno") as string;

    if (!CheckIfNoValue($(this).attr("data-createdname")))
        createdName = $(this).attr("data-createdname") as string;

    if (!CheckIfNoValue($(this).attr("data-submitteddate")))
        submittedDate = $(this).attr("data-submitteddate") as string;
...
function openEmployeeLookup() {
    ...
    if (!CheckIfNoValue($('input[name="StartDate"]').val()?.toString()))
        searchFilter.startDate = ConvertToISODate($('input[name="StartDate"]').val()!.toString()) as string;

    if (!CheckIfNoValue($('input[name="EndDate"]').val()?.toString()))
        searchFilter.endDate = ConvertToISODate($('input[name="EndDate"]').val()!.toString()) as string;

    if (!CheckIfNoValue($('select[name="ApprovalType"').val()))
        searchFilter.approvalType = $('select[name="ApprovalType"').val()!.toString();

    if (!CheckIfNoValue($("#hidEmpNo").val()?.toString()))
        searchFilter.empNo = GetIntValue($("#hidEmpNo").val()!.toString());

    if (!CheckIfNoValue($("#hidOtherEmpNo").val()?.toString()))
...
function onlyNumberKeyInq(evt: any) {
...
function displayErrorInquiry(obj: any, errText: any, focusObj: any) {
...
function searchButtonInquiry() {
    ...
    let startDate = $('input[name="StartDate"]').val()?.toString();
    let endDate = $('input[name="EndDate"]').val()?.toString();
