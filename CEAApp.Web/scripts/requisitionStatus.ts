...
function populateStatusTable(data: any) {
    var dataset = data;
...
function populateWFStatusTable(data: any) {
    var dataset = data;
    if (dataset == "undefined" || dataset == null) {
        // Get DataTable API instance
...
function displayErrorStatus(obj: any, errText: any, focusObj: any) {
...
function loadStatusDataTable() {
    try {
        ...
        if (!CheckIfNoValue($("#hidRequisitionID").val()?.toString()))
...
function loadWorkflowStatusTable() {
    try {
        ...
        if (!CheckIfNoValue($("#hidCEANo").val()?.toString()))
