...
var DateFormat;
(function (DateFormat) {
    DateFormat[DateFormat["ddMMyyyy"] = 0] = "ddMMyyyy";
    DateFormat[DateFormat["yyyyMMdd"] = 1] = "yyyyMMdd";
})(DateFormat || (DateFormat = {}));
var UserActionType;
(function (UserActionType) {
    UserActionType[UserActionType["NoAction"] = 0] = "NoAction";
    UserActionType[UserActionType["CancelRequest"] = 1] = "CancelRequest";
    UserActionType[UserActionType["ApproveRequest"] = 2] = "ApproveRequest";
    UserActionType[UserActionType["RejectRequest"] = 3] = "RejectRequest";
    UserActionType[UserActionType["ValidateRequest"] = 4] = "ValidateRequest";
    UserActionType[UserActionType["ReassignRequest"] = 5] = "ReassignRequest";
})(UserActionType || (UserActionType = {}));
var CEAStatusCode;
(function (CEAStatusCode) {
    CEAStatusCode["Draft"] = "Draft";
    CEAStatusCode["Submitted"] = "Submitted";
    CEAStatusCode["AwaitingChairmanApproval"] = "AwaitingChairmanApproval";
    CEAStatusCode["DraftAndSubmitted"] = "DraftAndSubmitted";
    CEAStatusCode["SubmittedForApproval"] = "SubmittedForApproval";
    CEAStatusCode["UploadedToOneWorld"] = "UploadedToOneWorld";
    CEAStatusCode["ChairmanApproved"] = "ChairmanApproved";
    CEAStatusCode["Closed"] = "Closed";
    CEAStatusCode["Approved"] = "Approved";
    CEAStatusCode["Rejected"] = "Rejected";
    CEAStatusCode["Cancelled"] = "Cancelled";
})(CEAStatusCode || (CEAStatusCode = {}));
// #endregion
// #region Classes
...
function ShowWaitMe(container, num, text) {
    var effect = '', maxSize = '', fontSize = '', color = '', // Color for background animation and text (string).
...
function GetDateValue(dtPicker, format) {
    var result = "";
...
function GetFormattedDate(inputDate, format) {
    var result = "";
...
function SaveFormData(exceptionList) {
    const formDataArray = [];
    // Save the value of all input text elements
    $(":text").each(function () {
        let controlKey = $(this).attr("name");
        let controlValue = $(this).val();
        let formData = { key: controlKey, value: controlValue, type: "text" };
    ...
    $("input[type=number]").each(function () {
        let controlKey = $(this).attr("name");
        let controlValue = $(this).val();
        let formData = { key: controlKey, value: controlValue, type: "number" };
        formDataArray.push(formData);
    });
    ...
    $("input[type=hidden]").each(function () {
        ...
        if (!CheckIfNoValue(controlKey)) {
            let formData = { key: controlKey, value: controlValue, type: "hidden" };
    ...
    $("select").each(function () {
        let controlKey = $(this).attr("name");
        let controlValue = $(this).val();
        let formData = { key: controlKey, value: controlValue, type: "select" };
        formDataArray.push(formData);
    });
    ...
    $("textarea").each(function () {
        let controlKey = $(this).attr("name");
        let controlValue = $(this).val();
        let formData = { key: controlKey, value: controlValue, type: "textarea" };
        formDataArray.push(formData);
    });
...
function RestoreFormData(dataArray) {
    ...
    if (Array.isArray(dataArray)) {
        dataArray.forEach(function (value, index) {
            let formData = { key: value.key, value: value.value, type: value.type };
