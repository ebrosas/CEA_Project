...
$(() => {
    ...
    if (callerFormVal == "RequisitionInquiry") {
        ...
        if (!(url === null || url === void 0 ? void 0 : url.includes("UserFunctions"))) {
    ...
    let projectNo = GetQueryStringValue("projectNo");
    if (CheckIfNoValue(projectNo)) {
        //let savedProjectNo = $("#hidProjectNo").val()?.toString();
        let savedProjectNo = (_a = $('input[name="ProjectNo"]').val()) === null || _a === void 0 ? void 0 : _a.toString();
...
function resetDetailForm() {
    try {
        ...
        let projectNo = $("#hidProjectNo").val();
        if (!CheckIfNoValue(projectNo))
            loadRequisitionTable(projectNo.trim(), false);
    }
    catch (error) {
...
function getRequisitionDetail() {
    let requisitionNo = $(this).text().trim();
...
function openCEAForm() {
    let projectNo = $("#hidProjectNo").val().trim();
    if (!CheckIfNoValue(projectNo)) {
...
function loadRequisitionTable(projectNo, restoreData, param) {
    try {
        const searchFilter = {
            projectNo: projectNo,
            requisitionNo: "",
