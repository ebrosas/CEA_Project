...
BEGIN
	...
	SELECT  		
		...
		a.CostCenter,
		RTRIM(g.CostCenterName) AS CostCenterName,		
		a.ProjectAmount AS ProjectBudget,
		d.RequestedAmt,
		h.DocumentType,
		h.BatchNo,
		h.DocumentNo,		
		CONVERT(VARCHAR, h.OrderDate, 103) AS GL_PO_Date,
		h.OrderNumber AS PurchaseOrderNo, 
		h.[LineNo] AS PurchaseOrderLineNo,
		h.Description1,
		h.Description2,
		(h.POAmount + h.GLAmount) AS AmountBD,
		h.InvoiceNo,
		--h.PaymentActualPaidAmount AS ActualPaidAmount,
		--h.PaymentDate,
		h.VendorNo, 
		h.VendorName,
		c.StatusCode AS ProjectStatus,
		f.StatusCode AS RequisitionStatus,
		a.FiscalYear
	...
	WHERE RTRIM(c.StatusCode) = 'Active'														--Show active projects only
		AND LTRIM(RTRIM(g.GroupCode)) NOT IN ('', 'S&M', 'ADM')									--Exclude admin cost centers
...
/*	Debug:
	...
	EXEC Projectuser.Pr_CapitalExpenseReport
