...
BEGIN 
	...
	BEGIN
		...
		SELECT	
				a.RequisitionID, 
				a.ProjectNo, 
				CASE WHEN ISNUMERIC(b.FiscalYear) = 1 THEN CAST(b.FiscalYear AS INT) ELSE 0 END AS FiscalYear,
				a.RequisitionNo, 
				...
				RTRIM(e.CEAWFInstanceID) AS WorkflowID	
		FROM dbo.Requisition a WITH (NOLOCK)
            ...
            (
				...
				SELECT	CASE WHEN ISNUMERIC(u.EmployeeNo) = 1 THEN CAST(u.EmployeeNo AS INT) ELSE 0 END AS AssignedToEmpNo,  
			...
			LEFT JOIN Projectuser.sy_CEAWF e WITH (NOLOCK) ON CASE WHEN ISNUMERIC(a.RequisitionNo) = 1 THEN CAST(a.RequisitionNo AS INT) ELSE 0 END = e.CEARequisitionNo
	...
	BEGIN
		...
		BEGIN
			...
			FROM dbo.Requisition a WITH (NOLOCK)
				...
				(
					...
					SELECT TOP 1
						CASE WHEN ISNUMERIC(u.EmployeeNo) = 1 THEN CAST(u.EmployeeNo AS INT) ELSE 0 END AS AssignedToEmpNo,  
						RTRIM(u.FullName) AS AssignedToEmpName,
						RTRIM(v.WFStatusCode) AS StatusCode, 
						RTRIM(w.UDCDesc1) AS StatusDesc, 
						RTRIM(w.UDCSpecialHandlingCode) AS StatusHandlingCode, 
					...
					SELECT
						...
						v.CEAStatusCode,
						v.CEAStatusDesc,
						RTRIM(x.CEAWFInstanceID) AS WorkflowID	   
					FROM Projectuser.sy_CEAWF x WITH (NOLOCK) 
        ...
        BEGIN
			...
			WHERE (RTRIM(a.RequisitionNo) = LTRIM(RTRIM(@requisitionNo)) OR @requisitionNo IS NULL)
				...
				AND 
				(	
					(a.CreatedByEmpNo = @empNo AND @createdByType = 1)		--Created by Me
					OR (RTRIM(b.CostCenter) = @userCostCenter AND a.CreatedByEmpNo <> @empNo AND @createdByType = 2)	--Created by Others
...
/*	Debug:
	...
	EXEC Projectuser.Pr_SearchRequisition '', '20230078'			--Filter by Requisition No.
	EXEC Projectuser.Pr_SearchRequisition '2220338', '20230049'		--Filter by Project No. and Requisition No.

	EXEC Projectuser.Pr_SearchRequisition null, null, null, 2022, 'DraftAndSubmitted', '7600', 0, null, null, null, null			--All Open Statuses	with fiscal year
	EXEC Projectuser.Pr_SearchRequisition null, null, null, 0, 'DraftAndSubmitted', '7600', 0, null, null, null, null						--All Open Statuses	
